...
public class EnsembleOffer {
    ...
    public void ConstructListe(){
        ...
        ResultSet rs = stmt.executeQuery("SELECT * FROM Product, Users WHERE ProductStatus = 0 AND Product.UserId = Users.UserId AND ProductName LIKE '%"+this.nameArticle+"%' ORDER BY ProductDate OFFSET "+(this.page*10)+" LIMIT 10 ");
        while (rs.next()) {
